...
import moovitLogo from '../images/Moovit-app-logo.png';
...
const SocialIcon = ({ name }: { name: string }) => {
  const icons: { [key: string]: JSX.Element } = {
    Moovit: (
      <img src={moovitLogo} alt="Moovit" className="w-8 h-8 object-contain" />
    ),
    Facebook: (
...
const VehicleMetrics = ({ data, periodFilter, filters, vehicleBenchmarks, selectedVehicle, onSelectVehicle, selectedPI }: VehicleMetricsProps) => {
  ...
  const vehicleData = useMemo(() => {
    ...
    filteredData.forEach(item => {
      ...
      const hasViewability = item.viewability !== undefined && item.viewability > 0;
      ...
      if (vehicleMap.has(key)) {
        ...
        if (hasViewability) {
          existing.viewabilitySum += item.viewability!;
          existing.viewabilityCount += 1;
        }
      } else {
        vehicleMap.set(key, {
          veiculo,
          ...
          viewabilitySum: hasViewability ? item.viewability! : 0,
          viewabilityCount: hasViewability ? 1 : 0
        });
      }
    });
    ...
    const vehicles: VehicleData[] = Array.from(vehicleMap.values()).map((metrics) => {
      ...
      const viewability = metrics.viewabilityCount > 0 ? metrics.viewabilitySum / metrics.viewabilityCount : 0;
              ...
              {vehicleData.map((vehicle, index) => (
                ...
                >
                  ...
                  <td className="py-3 px-4 text-center">
                    <span className="font-semibold text-gray-700">
                      {vehicle.viewability > 0 ? `${(vehicle.viewability * 100).toFixed(2)}%` : '—'}
                    </span>
                  </td>
                </tr>
              ))}
