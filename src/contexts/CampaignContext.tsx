...
export const CampaignProvider = ({ children }: CampaignProviderProps) => {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        const correctedCampaignData = campaignData.map(item => ({
          ...item,
          numeroPi: item.numeroPi ? (item.numeroPi.replace(/^0+/, '') || '0') : item.numeroPi
        }));
  ...
  const calculateMetrics = (dataSet: ProcessedCampaignData[]): CampaignMetrics => {
    ...
    // Viewability: média dos valores disponíveis (ignorando zeros/undefined)
    const viewabilityRows = dataSet.filter(row => row.viewability !== undefined && row.viewability > 0);
    const avgViewability = viewabilityRows.length > 0
      ? viewabilityRows.reduce((acc, row) => acc + (row.viewability ?? 0), 0) / viewabilityRows.length
      : 0;
